...
For batch inference on directories of images, use the `inference.py` CLI script. This provides a command-line interface for running RF-DETR models on multiple images at once, with support for various output formats.

**Basic usage:**

```bash
python inference.py <images_dir> <weights_path> <output_dir> [options]
```

**Example - Segmentation with visualization:**
...
python inference.py \
    ...
    --visualize \
    --save-coco-labels \
    --class-names=datasets/class_names.txt \
    --conf=0.3 \
    --device=cuda:0
```
...
- `--device=<device>`: Device - cpu, cuda, mps, cuda:0, etc. (default: cpu)
- `--segmentation`: Enable segmentation head for instance segmentation models
- `--visualize`: Save visualization images with overlaid masks/boxes
- `--save-txt`: Save segmentation polygons as text files
- `--save-coco-labels`: Save COCO format JSON annotations
- `--class-names=<path>`: Path to class names file (one class per line)
- `--num-classes=<n>`: Number of classes (required if no class-names file)
- `--filter-classes=<ids>`: Filter by specific class IDs (comma-separated)
...
See [INFERENCE_EXAMPLES.md](INFERENCE_EXAMPLES.md) for more detailed usage examples.
